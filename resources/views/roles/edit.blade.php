                ...
                <form class="form-horizontal" method="POST" action="{{ route('roles.update',$role->id) }}">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-5">
                                <input type="text" name="name" value="{{ $role->name }}" class="form-control" id="key" placeholder="Enter Name">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="guard_name" class="col-sm-2 col-form-label">Guard Name</label>
                            <div class="col-sm-5">
                                <input type="text" value="{{ $role->guard_name }}" name="guard_name" class="form-control" id="guard_name" placeholder="Guard name">
                            </div>
                        </div>
                        <div class="form-group row">
                            @foreach(auth_list('permissions') as $permission)
                                <div class="custom-control custom-switch m-2">
                                    ...
                                    <input type="checkbox" name="permission[]"
                                           @foreach($role->permissions->pluck('name') as $pname)
                                               @if($pname == $permission)
                                                checked
                                               @endif
                                           @endforeach
